...
- [ ] What problem does the context API help solve?
      ...
      Context API helps solve state management issues, and eliminates the need for props drilling

- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

      Actions are packets of information that contain an action type and associated data

      Reducers are pure functions (they don't produce side-effects). They never update state directly, they only return a new object

      The store is everything that changes in our application, represented in a single JS object. It contains application state

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?


      Application state is state that the entire APP shares.....Component state is state that is relevant to a specific component 

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

      Redux thunk is middleware, and it can stop actions, forward an action untouched, dispatch a different action, and dispatch multiple actions

- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

Redux I think I like better than Context. I think it's just because I used it more, so it's getting more familiar
